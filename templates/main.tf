...
terraform {
  backend "azurerm" {
    resource_group_name   = "dev13"
    storage_account_name  = "storageiesgn1"
...
resource "azurerm_virtual_machine_extension" "az_vm_extension" {
  ...
  settings = <<SETTINGS
    {
      "commandToExecute": "apt-get update && apt-get install -y apache2 git && git clone ${var.app_repo_url} /var/www/html/ && systemctl restart apache2"
    }
  SETTINGS
